    ...
    <div className='w-full '>
      ...
      {sort.map((item, idx) => (
                            <div key={idx + 1} onClick={() => handleButtonClick(item)}
                                ...
                                ${label == item  ?  'bg-blue-500 ' : 'bg-[#EAEAEA]'}  duration-300 transition-all rounded-full flex 
      ...
      }} className='bg-blue-500 my-1 px-1 py-2 w-full
       rounded-[15px] text-white'>Show</button>
        </div> 
    </div>
  </div>
</Modalview>
